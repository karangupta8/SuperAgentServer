...
class AcpMessage(BaseModel):
    sender_agent_id: str = Field(
        ..., description="The unique ID of the sending agent."
    )
    message: str = Field(..., description="The content of the message.")
    session_id: str | None = Field(
        None, description="An optional session ID for the conversation."
    )
...
@router.post("/message")
async def acp_message(
    message: AcpMessage, agent: BaseAgent = Depends(get_agent)
):
...
async def get_manifest(agent: BaseAgent):
    ...
    return {
        "name": "ACP Adapter",
        "description": (
            "Allows the agent to communicate over a message broker "
            "(e.g., RabbitMQ)."
        ),
